-- Drop all possible variants of functions to avoid conflicts
-- This will clean up any existing functions completely

-- Drop create_user_profile variants
...
DROP FUNCTION IF EXISTS create_user_profile(UUID);

-- Drop admin_update_profile variants
DROP FUNCTION IF EXISTS admin_update_profile(UUID, JSONB);
DROP FUNCTION IF EXISTS admin_update_profile(UUID, TEXT);
DROP FUNCTION IF EXISTS admin_update_profile(UUID);

-- Drop update_user_balance variants (all possible signatures)
...
DROP FUNCTION IF EXISTS update_user_balance(UUID, NUMERIC, TEXT, TEXT);
DROP FUNCTION IF EXISTS update_user_balance(UUID, NUMERIC, TEXT);
DROP FUNCTION IF EXISTS update_user_balance(UUID, NUMERIC);

-- Drop complete_user_task variants
DROP FUNCTION IF EXISTS complete_user_task(UUID, UUID, DECIMAL);
DROP FUNCTION IF EXISTS complete_user_task(UUID, UUID, NUMERIC);
DROP FUNCTION IF EXISTS complete_user_task(UUID, UUID);

-- Drop update_user_xp variants
DROP FUNCTION IF EXISTS update_user_xp(UUID, INTEGER);
DROP FUNCTION IF EXISTS update_user_xp(UUID, INT);
DROP FUNCTION IF EXISTS update_user_xp(UUID);

-- Drop any other potential function variants
DROP FUNCTION IF EXISTS admin_get_all_profiles();
DROP FUNCTION IF EXISTS generate_referral_code();

-- Now recreate the generate_referral_code function first (dependency)
...
-- Recreate update_user_balance function with unique signature
...
-- Recreate complete_user_task function
...
BEGIN
  ...
  -- Update XP
  PERFORM update_user_xp(p_user_id, 10);
  
  RETURN TRUE;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Recreate update_user_xp function
