-- Add sample data for testing all features (FIXED VERSION)

-- Insert sample tasks/offers
INSERT INTO tasks (title, description, payout, category, provider, difficulty, estimated_time, countries, devices, requirements, is_active, url) VALUES
('Complete Survey - Shopping Habits', 'Answer questions about your shopping preferences and earn points', 50, 'survey', 'CPX Research', 'easy', '5 minutes', ARRAY['US', 'CA', 'UK', 'AU'], ARRAY['mobile', 'desktop'], ARRAY['Age 18+', 'Valid email'], true, 'https://example.com/survey1'),
('Download and Play Game', 'Download Candy Crush and reach level 10', 150, 'gaming', 'AdGem', 'medium', '15 minutes', ARRAY['US', 'CA'], ARRAY['mobile'], ARRAY['Android 8+', 'iOS 12+'], true, 'https://example.com/game1'),
('Sign up for Newsletter', 'Subscribe to our partner newsletter and confirm email', 25, 'email', 'Lootably', 'easy', '2 minutes', ARRAY['US', 'CA', 'UK', 'AU', 'DE'], ARRAY['mobile', 'desktop'], ARRAY['Valid email'], true, 'https://example.com/newsletter1'),
('Watch Video Series', 'Watch 5 short videos about finance', 75, 'video', 'OfferToro', 'easy', '10 minutes', ARRAY['US', 'CA', 'UK'], ARRAY['mobile', 'desktop'], ARRAY['Age 16+'], true, 'https://example.com/video1'),
('Install Shopping App', 'Download and register on shopping app', 100, 'app', 'BitLabs', 'medium', '8 minutes', ARRAY['US', 'CA', 'UK', 'AU'], ARRAY['mobile'], ARRAY['Android 7+', 'iOS 11+'], true, 'https://example.com/app1'),
('Complete Profile Survey', 'Fill out detailed profile information', 200, 'survey', 'Revenue Universe', 'hard', '20 minutes', ARRAY['US', 'CA', 'UK', 'AU', 'DE', 'FR'], ARRAY['mobile', 'desktop'], ARRAY['Age 21+', 'Valid phone'], true, 'https://example.com/survey2'),
('Try Free Trial', 'Sign up for 7-day free trial service', 300, 'trial', 'Persona.ly', 'medium', '5 minutes', ARRAY['US', 'CA', 'UK'], ARRAY['mobile', 'desktop'], ARRAY['Credit card required', 'Age 18+'], true, 'https://example.com/trial1'),
('Social Media Follow', 'Follow our partner on social media', 15, 'social', 'AyeT Studios', 'easy', '1 minute', ARRAY['US', 'CA', 'UK', 'AU', 'DE', 'FR', 'ES', 'IT'], ARRAY['mobile', 'desktop'], ARRAY['Social media account'], true, 'https://example.com/social1');
...
BEGIN
    ...
    FOR profile_record IN (SELECT id FROM profiles LIMIT 2) LOOP
        FOR task_record IN (SELECT id, payout FROM tasks LIMIT 3) LOOP
...
AND NOT EXISTS (
    ...
    WHERE tr.reference_id = ut.id::text
);
...
SELECT 
  id as user_id,
  500 as amount, -- $5.00 worth of points
  'DANA' as method,
  '{"phone": "+628123456789", "name": "John Doe"}'::jsonb as account_details,
  'completed' as status,
...
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'user_badges') 
       AND EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'badges') THEN
...
DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'user_levels') THEN
        INSERT INTO user_levels (user_id, current_level, current_xp, total_xp, level_name, next_level_xp)
        SELECT 
            user_id,
            2 as current_level,
            150 as current_xp,
            150 as total_xp,
            'Bronze' as level_name,
            300 as next_level_xp
        FROM user_tasks 
...
-- Create some fraud logs for testing (admin features) - if table exists
DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'fraud_logs') THEN
        INSERT INTO fraud_logs (user_id, type, details, confidence_score, risk_level)
        SELECT 
          id as user_id,
          'suspicious_activity' as type,
          '{"reason": "Multiple rapid completions", "count": 5}'::jsonb as details,
...
DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'daily_bonuses') THEN
...
SELECT 'Tasks inserted:' as info, COUNT(*) as count FROM tasks;
SELECT 'Profiles with data:' as info, COUNT(*) as count FROM profiles WHERE points > 0;
SELECT 'User tasks completed:' as info, COUNT(*) as count FROM user_tasks WHERE status = 'completed';
SELECT 'Transactions created:' as info, COUNT(*) as count FROM transactions;
SELECT 'Withdrawals created:' as info, COUNT(*) as count FROM withdrawals;
