-- 🔧 FIX USER_TASKS TABLE SCHEMA (CORRECTED VERSION)
...
-- 2. Make foreign keys NOT NULL (only if you want this - OPTIONAL)
-- Skip this if you want to keep nullable foreign keys
...
-- ✅ FIX: Use date_trunc instead of DATE() for IMMUTABLE function
CREATE INDEX IF NOT EXISTS idx_user_tasks_daily_completions 
ON public.user_tasks USING btree (user_id, date_trunc('day', completed_at)) 
TABLESPACE pg_default
...
BEGIN
  ...
  -- Prevent duplicate completions (optional - remove if you allow re-attempts)
...
-- 7. Create trigger for validation (optional)
...
BEGIN
  ...
  SELECT 
    ...
    COUNT(*) FILTER (WHERE status = 'completed' AND date_trunc('day', completed_at) = date_trunc('day', CURRENT_TIMESTAMP)) as today_completed,
    COALESCE(SUM(reward_earned) FILTER (WHERE status = 'completed' AND date_trunc('day', completed_at) = date_trunc('day', CURRENT_TIMESTAMP)), 0) as today_earned,
    COUNT(*) FILTER (WHERE status = 'completed' AND completed_at >= date_trunc('week', CURRENT_TIMESTAMP)) as this_week_completed,
    COALESCE(SUM(reward_earned) FILTER (WHERE status = 'completed' AND completed_at >= date_trunc('week', CURRENT_TIMESTAMP)), 0) as this_week_earned
  FROM user_tasks
