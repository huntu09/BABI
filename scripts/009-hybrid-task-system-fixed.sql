-- 🔄 HYBRID TASK SYSTEM - PHASE 2 (FIXED)
-- Add columns and tables for hybrid sample + real offerwall system

-- Add missing columns to tasks table
ALTER TABLE tasks ADD COLUMN IF NOT EXISTS difficulty VARCHAR(20) DEFAULT 'easy';
ALTER TABLE tasks ADD COLUMN IF NOT EXISTS estimated_time VARCHAR(50) DEFAULT '10 minutes';
...
UPDATE tasks SET task_source = 'sample' WHERE task_source IS NULL OR task_source = '';
...
-- Update existing sample tasks with difficulty and estimated_time
UPDATE tasks SET 
    difficulty = CASE 
        WHEN reward_amount <= 0.50 THEN 'easy'
        WHEN reward_amount <= 1.50 THEN 'medium'
        ELSE 'hard'
    END,
    estimated_time = CASE 
        WHEN reward_amount <= 0.50 THEN '5 minutes'
        WHEN reward_amount <= 1.50 THEN '10 minutes'
        ELSE '15 minutes'
    END
WHERE difficulty IS NULL OR estimated_time IS NULL;
...
INSERT INTO tasks (
    ...
    is_active, url, task_source, external_id, provider_config
) VALUES 
(
    ...
    'easy', '5 minutes', 
    ARRAY['US', 'GB', 'CA'], ARRAY['mobile', 'desktop'], '["Age 18+"]'::jsonb,
...
(
    ...
    'medium', '8 minutes',
    ARRAY['US', 'GB', 'CA', 'AU'], ARRAY['mobile', 'desktop'], '["Age 18+", "Tech user"]'::jsonb,
...
(
    ...
    'medium', '12 minutes',
    ARRAY['US', 'GB'], ARRAY['mobile', 'desktop'], '["Age 21+", "Health conscious"]'::jsonb,
